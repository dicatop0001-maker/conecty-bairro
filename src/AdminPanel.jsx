...
const S = {
  ...
  card: (border) => ({ background:'#f8fafc',border:'2px solid '+(border||'#e2e8f0'),borderRadius:'14px',padding:'12px 14px',marginBottom:'10px',display:'flex',gap:'12px',alignItems:'center',cursor:'pointer' }),
  btnDel: { padding:'7px 14px',background:'#dc2626',color:'#fff',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'800',fontSize:'12px' },
  btnOk: { padding:'7px 14px',background:'#16a34a',color:'#fff',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'800',fontSize:'12px' },
  btnEdit: { padding:'7px 14px',background:'#f59e0b',color:'#fff',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'800',fontSize:'12px' },
  btnBlock: { padding:'7px 14px',background:'#7c3aed',color:'#fff',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'800',fontSize:'12px' },
...
function Confirm({ msg, onYes, onNo }) {
  return ReactDOM.createPortal(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:199999,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}>
      <div style={{background:'#fff',borderRadius:'16px',padding:'24px',maxWidth:'340px',width:'100%',textAlign:'center',boxShadow:'0 8px 32px rgba(0,0,0,0.3)'}}>
        <div style={{fontSize:'36px',marginBottom:'10px'}}>⚠️</div>
        <p style={{fontSize:'15px',fontWeight:'700',color:'#1e293b',marginBottom:'18px'}}>{msg}</p>
        <div style={{display:'flex',gap:'10px'}}>
          <button onClick={onNo} style={{flex:1,padding:'11px',background:'#f1f5f9',border:'none',borderRadius:'10px',cursor:'pointer',fontWeight:'700',fontSize:'14px'}}>Cancelar</button>
          <button onClick={onYes} style={{flex:2,padding:'11px',background:'#dc2626',color:'#fff',border:'none',borderRadius:'10px',cursor:'pointer',fontWeight:'800',fontSize:'14px'}}>Confirmar</button>
        </div>
      </div>
    </div>, document.body
  )
...
function AdminPanel({ onClose, adminUser }) {
  ...
  const [data, setData] = useState({ anuncios:[], leiloes:[], sponsors:[], pros:[], assoc:[], blocked:[] })
  ...
  const load = async () => {
    ...
    try {
      const [an, le, sp, pr, bl] = await Promise.all([
        ...
        supabase.from('blocked_users').select('*').order('created_at',{ascending:false}).maybeSingle().then(()=>
  ...
  // EDIT SPONSOR MODAL
  if(editSp) return ReactDOM.createPortal(
      ...
      <div style={S.panel} onClick={e=>e.stopPropagation()}>
        ...
        <div style={S.body}>
          ...
          {!loading && tab==='anuncios' && (
            data.anuncios.length===0 ? <div style={S.empty}><div style={{fontSize:'40px'}}>📢</div><div style={{fontWeight:'700',marginTop:'8px'}}>Nenhum anúncio</div></div>
            : data.anuncios.map(item=>(
              <div key={item.id} style={S.card('#fed7aa')}>
                {item.images&&item.images[0]
                  ? <img src={item.images[0]} style={{width:'52px',height:'52px',objectFit:'cover',borderRadius:'8px',flexShrink:0}} alt='' />
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:'10px',color:'#94a3b8',marginTop:'2px'}}>{fmt(item.created_at)} • ID: {item.user_id?.substring(0,8)||'—'}</div>
                </div>
                <div style={{display:'flex',flexDirection:'column',gap:'5px',flexShrink:0}}>
                  <button style={S.btnDel} onClick={()=>del('auctions',item.id,'anúncio')}>🗑️ Excluir</button>
                  <button style={S.btnBlock} onClick={()=>blockUser(item.user_id,null)}>🚫 Bloquear</button>
                </div>
              </div>
            ))
          )}
          ...
          {/* LEILOES */}
          {!loading && tab==='leiloes' && (
            data.leiloes.length===0 ? <div style={S.empty}><div style={{fontSize:'40px'}}>🔨</div><div style={{fontWeight:'700',marginTop:'8px'}}>Nenhum leilão</div></div>
              ...
              return (
                <div key={item.id} style={S.card(ended?'#e2e8f0':'#86efac')}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:'10px',color:ended?'#dc2626':'#16a34a',marginTop:'2px',fontWeight:'700'}}>{ended?'⏹ Encerrado':'🟢 Ativo'} • Expira: {fmt(item.ends_at)}</div>
                    <div style={{fontSize:'10px',color:'#94a3b8'}}>ID dono: {item.user_id?.substring(0,8)||'—'}</div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:'5px',flexShrink:0}}>
                    <button style={S.btnDel} onClick={()=>del('auctions',item.id,'leilão')}>🗑️ Excluir</button>
                    <button style={S.btnBlock} onClick={()=>blockUser(item.user_id,null)}>🚫 Bloquear</button>
                  </div>
                </div>
              )
          ...
          {!loading && tab==='sponsors' && (
            data.sponsors.length===0 ? <div style={S.empty}><div style={{fontSize:'40px'}}>⭐</div><div style={{fontWeight:'700',marginTop:'8px'}}>Nenhum patrocinador</div></div>
          ...
          {/* PROFISSIONAIS */}
          {!loading && tab==='pros' && (
            data.pros.length===0 ? <div style={S.empty}><div style={{fontSize:'40px'}}>👷</div><div style={{fontWeight:'700',marginTop:'8px'}}>Nenhum profissional</div></div>
            : data.pros.map(pro=>(
            ...
            <div>
              <div style={{background:'#fef2f2',border:'2px solid #fca5a5',borderRadius:'12px',padding:'12px 14px',marginBottom:'14px'}}>
                ...
                <div style={{display:'flex',gap:'8px'}}>
                  ...
                  />
                  <button
                    style={{...S.btnDel,padding:'9px 14px',whiteSpace:'nowrap'}}
                    onClick={()=>{
                      const v = blockInput.trim()
                      if(!v) return
                      const isEmail = v.includes('@')
                      blockUser(isEmail?null:v, isEmail?v:null)
                    }}
                  >🚫 Bloquear</button>
                </div>
                <div style={{fontSize:'11px',color:'#dc2626',marginTop:'6px',fontWeight:'600'}}>⚠️ Cole o ID do usuário (UUID) ou o e-mail. O usuário bloqueado não poderá publicar ou fazer lances.</div>
              </div>
              {data.blocked.length===0
                ...
                : data.blocked.map(b=>(
                  <div key={b.id} style={S.card('#fca5a5')}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:'800',fontSize:'13px',color:'#1e293b'}}>{b.email||b.user_id||'ID desconhecido'}</div>
                      <div style={{fontSize:'10px',color:'#94a3b8',marginTop:'2px'}}>Bloqueado em: {fmt(b.blocked_at)}</div>
                    </div>
        ...
        <div style={{padding:'8px 16px',borderTop:'1px solid #e2e8f0',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0,background:'#f8fafc'}}>
          ...
          <button onClick={onClose} style={{padding:'7px 16px',background:'#1e3a8a',color:'#fff',border:'none',borderRadius:'10px',cursor:'pointer',fontWeight:'700',fontSize:'12px'}}>Fechar</button>
        </div>
